...
public class SuperWiFi extends MainActivity{
     ...
     * ***************************************************************************************************************/
    private String FileLabelName = "薛冯晨";// Define the file Name
    /*****************************************************************************************************************
     ...
     * ***************************************************************************************************************/
    private String FileNameGroup[] = {"ChinaNet-1602","TP-1602"};
    ...
    public SuperWiFi(Context context)
    {
        ...
        this.position = new Vector<String>();
    }
        ...
        Thread scanThread = new Thread(new Runnable()
        {
            public void run() {
                ...
                for(int index = 1;index <= NumberOfWiFi; index++){
                    ...
                    Log.d("TEST_INFO",FileLabelName + "-" + FileNameGroup[index - 1] + ".txt" + "Test_ID: " + testID + " TestTime: " + CurTimeString + " BEGIN\r\n");
                }
                //Scan for a certain times
                Log.d("test","scanning start1");
                while(CurTestTime++ <= TestTime) performScan();
                Log.d("test","scanning start2");
                for(int index = 1;index <= NumberOfWiFi; index++){//Record the average of the result
                    ...
                    double tmp = RSSI2Distance(RSS_Value_Record[index - 1] / RSS_Measurement_Number_Record[index - 1]);
                    position.add(df.format(tmp) + "m ");
                    //Log.d("XFCNBNB!!!"," " + RSSI2Distance(RSS_Value_Record[index-1]));
                }
                Log.d("test","scanning end");
                /*****************************************************************************************************************
                ...
                for(int index = 1;index <= NumberOfWiFi; index++){//Mark the end of the test in the file
                   // write2file(FileLabelName + "-" + FileNameGroup[index - 1] + ".txt","testID:"+testID+"END\r\n");
                    Log.d("TEST_INFO","testID:"+testID+"END\r\n");
                }
                isScanning=false;
            }
        });
    ...
    private double RSSI2Distance(int RSSI){
        double A = 38;
        double n = 2.0;
        int mRSSI = Math.abs(RSSI);
        return Math.pow(10,(double)(mRSSI - A)/(10*2.0));
    }
