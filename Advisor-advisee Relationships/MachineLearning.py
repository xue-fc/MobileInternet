...
def SVM(X_train,y_train,X_test,y_test):
    ...
    print("准确率：",score)
    print("召回率：",metrics.recall_score(expected, predicted))
    return model,score
    #print (data.head())
...
def simple_deep_learning_model(X_train,y_train,X_test,y_test,X_all,shape_in):
    ...
    model.fit(X_train, y_train, nb_epoch=5, batch_size=5, verbose=0)
    print ("training time:", round(time()-t0, 3), "s")
    t0 = time()
...
#SVM模型训练
#model_deep_learning = SVM(X_train,y_train,X_test,y_test)

#决策树模型训练
#model_deep_learning = DTree(X_train,y_train,X_test,y_test)
